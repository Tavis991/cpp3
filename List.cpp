...
const Drone& List::operator[](int index) const {
	if (index < 0 || index >= count) {
		cerr << "whoa there buddy" << endl;
		exit(1);
	}
...
List::Node::Node(Drone data, Node* next) : data(data), next(next) {}
